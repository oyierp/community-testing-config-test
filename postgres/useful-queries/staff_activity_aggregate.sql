...
SELECT
	...
	SUM(COALESCE(FORWARDED_CASES.count,0)) AS forwarded_cases,
	SUM(COALESCE(ACTIVE_STAFF.active,0)) AS  users_active,
...
FROM (
	...
	generate_series( '2020-07-20'::date,  '2020-07-26'::date, interval '1 day')  as t(d)
...
LEFT JOIN (
	...
	GROUP BY
		1,2) AS FORWARDED_CASES ON STAFF.county_id = FORWARDED_CASES.county_id AND STAFF.day = FORWARDED_CASES.day
